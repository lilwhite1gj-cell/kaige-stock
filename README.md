...
- **AI 智能分析（可选开关）**：
  ...
  - **📊 ETF 板块推荐（聚焦两类市场）**：精选 ETF 池覆盖 **国内板块 ETF**（宽基核心 / 行业 / 策略）与 **纳斯达克相关 ETF**（纳指100 / 标普500 / 道琼斯），DeepSeek 结合新闻面与实时量价，对每只 ETF 给出 **操作建议（买入 / 持有 / 回避 / 定投）** 与 **推荐逻辑**，并按板块分类展示；价格每 30 秒实时刷新。
...
- **ETF 精选池**：`src/config.js` 的 `etf.domestic` / `etf.nasdaq` 可配置；分别聚焦国内板块与纳斯达克相关 ETF，每项含 `secid`（东方财富代码）/ `name` / `code` / `sector`（板块分类）。
...
src/
  ...
  fetchQuotes.js       实时行情抓取（东方财富 + 腾讯兜底 + 大盘指数 + ETF 池）
...
| `/api/etf-quotes` | GET | 获取精选 ETF 池实时行情（国内 + 纳斯达克相关），无需 Key，前端 30s 轮询 |
| `/api/recommendations` | GET | 获取综合荐股结果（新闻 × 行情融合，需配置 Key） |
| `/api/refresh` | POST | 手动触发重新抓取 + 分析（含 ETF）+ 行情 + 融合 |
